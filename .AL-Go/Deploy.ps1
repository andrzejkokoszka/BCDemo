...
try {
    ...
    if ($searchArtifacts) {
        ...
        $allArtifacts = @(GetArtifacts -token $token -api_url $ENV:GITHUB_API_URL -repository $ENV:GITHUB_REPOSITORY -mask "Apps" -projects $projects -Version $artifacts -branch $ENV:GITHUB_REF_NAME)
        $allArtifacts += @(GetArtifacts -token $token -api_url $ENV:GITHUB_API_URL -repository $ENV:GITHUB_REPOSITORY -mask "Dependencies" -projects $projects -Version $artifacts -branch $ENV:GITHUB_REF_NAME)
        if ($allArtifacts) {
            ...
            throw "Could not find any Apps artifacts for projects $projects, version $artifacts, refname= $ENV:GITHUB_REF_NAME"
        }
    }
